...
def create_rnn_model(fingerprint_input, model_settings, is_training):
  ...
  model.add(layers.InputLayer(input_shape=(n_chunks, chunk_size)))
  if is_training:
    model.add(layers.LSTM(rnn_size1, return_sequences=True, kernel_regularizer=reg, recurrent_regularizer=reg))  
  else:
    model.add(layers.LSTM(rnn_size1, return_sequences=True))
  ...
  if is_training:
    model.add(layers.LSTM(rnn_size1, kernel_regularizer=reg, recurrent_regularizer=reg)) 
  else:
    model.add(layers.LSTM(rnn_size1))
  model.add(layers.Dense(label_count))
  
  print(model.summary())

  logits = model(fingerprint_3d) 
